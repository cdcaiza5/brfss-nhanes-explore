...
For the execution of the study, the CRISP-DM (Cross-Industry Standard Process for Data Mining) methodology \cite{chapman2000crisp} %[cite: 3] 
...
Feature selection was performed by training a Random Forest classifier on a $100{,}000$-row subsample over the full $301$-column universe, ranking variables by Gini importance. Eleven variables were excluded from the universe (Table~\ref{tab:excluded}); the canonical predictor set is the top-$30$ by RF importance, with two documented substitutions: \texttt{\_MENT14D} (RF rank $2$) was excluded because it is a near-duplicate of the target \texttt{ADDEPEV3}, and \texttt{\_STSTR} (RF rank $30$) was excluded because it is a sampling stratum rather than a predictor. \texttt{\_PACKYRS} (RF rank $36$) and \texttt{MEDCOST1} (RF rank $47$) were substituted in to replace them. The literal next two RF ranks ($31$ and $32$, importances $0.00605$ and $0.00572$) were not used: rank $31$ is the mean-encoded form of \texttt{\_RACE}, which is represented separately, and rank $32$ (\texttt{\_CASTHM1}) is a redundant legacy variant of \texttt{\_ASTHMS1} at rank $17$. The $100{,}000$-row subsample was used for the feature-selection step alone (Random Forest fitting, mutual information, and permutation importance are not tractable on $455{,}006$ rows for the full universe); the chosen predictor set was then trained and evaluated on the full $455{,}006$-row dataset in the $5$-fold cross-validation. The chosen set contains $30$ raw predictors ($8$ numeric, $4$ ordinal, $18$ categorical), with \texttt{\_RACE} mean-encoded separately; the model therefore receives $31$ input features per row. The relevant RF ranking is included in Appendix~\ref{app:rf} %[cite: 1].

\begin{table}[!htbp]
\caption{Variables excluded from the feature-selection universe}
\label{tab:excluded}
\centering
\scriptsize
...
\hline
\textbf{Variable} & \textbf{Category} & \textbf{Reason for exclusion} \\
\hline
ADDEPEV3   & target        & Lifetime depression diagnosis; the prediction target. \\
\_STATE    & geography     & U.S. state of residence. \\
SEQNO      & identifier    & Respondent sequence number. \\
\_PSU      & identifier    & Primary sampling unit. \\
\_STRWT    & design weight & Design stratum weight. \\
\_WT2RAKE  & design weight & Final raking weight. \\
\_LLCPWT   & design weight & Landline-cellphone design weight. \\
\_LLCPWT2  & design weight & Design weight (LLCP 2024). \\
\_CLLCPWT  & design weight & Combined landline/cellphone weight. \\
IDATE      & structural    & Interview date. \\
WEIGHT2    & redundant     & Reported weight in pounds; redundant with WTKG3. \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\caption{Top-30 Predictors of the Chosen Set (RF-ranked)}
\begin{center}
\scriptsize
\setlength{\tabcolsep}{3pt}
...
In addition to the standard probability metrics, the per-model decision threshold was tuned out-of-fold by maximizing F1; alternatives such as Youden's $J$ statistic and a cost-sensitive criterion (with a user-defined cost ratio) are also applicable for screening and cost-sensitive applications. The reported \texttt{*@thr} columns show metrics at the chosen threshold, and the \texttt{threshold} column reports the threshold itself. In this study we report the F1-tuned threshold (5-fold mean). %[cite: 1]
...
The findings of the predictive modeling experiment are presented based on the evaluation of five machine learning architectures over a $5$-fold stratified cross-validation process on the full $455{,}006$-row dataset. The algorithms were evaluated on their ability to detect the \texttt{ADDEPEV3} target using the $30$ socio-behavioral biomarkers of the chosen predictor set. Thresholds are tuned per model out-of-fold to maximize F1; the \texttt{*@thr} columns report metrics at the chosen threshold, while the unsubscripted accuracy is reported at the $0.5$ default threshold.
...
Table~\ref{app:rf} lists the Random Forest Gini-importance top-30 of the chosen predictor set, plus the two substituted-in variables from later RF ranks. Rows marked with a single asterisk are excluded from the chosen set; rows marked with a double asterisk are substituted in.
...
47 & MEDCOST1$^{**}$   & 0.00380 \\
\hline
\multicolumn{3}{|p{8cm}|}{\footnotesize $^{\mathrm{a}}$\textit{Note.} $^{*}$Excluded from the chosen set by documented substitution: \texttt{\_MENT14D} (rank 2) as a near-duplicate of the target, \texttt{\_STSTR} (rank 30) as a sampling stratum. $^{**}$Substituted in from later RF ranks to replace the excluded rows: \texttt{\_PACKYRS} (rank 36), \texttt{MEDCOST1} (rank 47).} \\
\hline
\end{tabular}
\end{table}
